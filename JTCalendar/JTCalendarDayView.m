...
- (void)commonInit
{
    ...
    {
        circleBottomView = [UIView new];
        circleBottomView.backgroundColor = [UIColor colorWithRed:250.0/255.0 green:250.0/255.0 blue:255.0/255.0 alpha:1];
        [self addSubview:circleBottomView];
    }
    
    {
        circleView = [JTCircleView new];
        [circleBottomView addSubview:circleView];
    }
    
    {
        textLabel = [UILabel new];
        [circleBottomView addSubview:textLabel];
    }
...
{
    textLabel.frame = CGRectMake(0, 0, self.frame.size.height, self.frame.size.height);
    ...
    circleView.center = CGPointMake(self.frame.size.height / 2., self.frame.size.height / 2.);
    [circleView kc_setLayerCornerRadiusWithClips:sizeCircle / 2];
    ...
    [dotView kc_setLayerCornerRadiusWithClips:sizeDot / 2.];
    
    circleBottomView.frame = CGRectMake((self.frame.size.width - self.frame.size.height)/2, 0, self.frame.size.height, self.frame.size.height);
...
- (void)reloadAppearance
{
    ...
    
    {
        if (self.isOtherMonth) {
            ...
            [circleBottomView kc_setBorderWithWidth:1 cornerRadius:self.frame.size.height/2 color:[UIColor whiteColor] roundingCorners:UIRectCornerAllCorners];
            return;
        }
        UIColor *borderColor = HRTextImportColor01;
        if ([self.calendarManager.dateDict objectForKey:textLabel.text]) {
            if ([[self.calendarManager.dateDict objectForKey:textLabel.text] isEqualToString:@"补签"]) {
                borderColor = HRRainbowColor04;
            } else if ([[self.calendarManager.dateDict objectForKey:textLabel.text] isEqualToString:@"加班"]) {
                borderColor = HRRainbowColor01;
            } else if ([[self.calendarManager.dateDict objectForKey:textLabel.text] isEqualToString:@"请假"]) {
                borderColor = HRRainbowColor02;
            } else if ([[self.calendarManager.dateDict objectForKey:textLabel.text] isEqualToString:@"迟到"]) {
                borderColor = HRRainbowColor03;
            } else if ([[self.calendarManager.dateDict objectForKey:textLabel.text] isEqualToString:@"早退"]) {
                borderColor = HRRainbowColor05;
            } else if ([[self.calendarManager.dateDict objectForKey:textLabel.text] isEqualToString:@"未刷卡"]) {
                borderColor = HRRainbowColor06;
            } else if ([[self.calendarManager.dateDict objectForKey:textLabel.text] isEqualToString:@"出差"]) {
                borderColor = HRRainbowColor07;
            }
            [circleBottomView kc_setBorderWithWidth:1 cornerRadius:self.frame.size.height/2 color:borderColor roundingCorners:UIRectCornerAllCorners];
        } else {
            if (self.frame.size.height == 0) {
                return;
            }
